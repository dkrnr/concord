...
function mount(canvas:HTMLCanvasElement,initialRoom:Room,reduced:boolean,initialDevices:DeviceState):SceneApi{
  ...
  const draw=()=>{if(disposed)return;const desired=CAMERA[desiredRoom],speed=reduced?1:.075;camera.position.lerp(desired.position,speed);target.lerp(desired.target,speed);camera.zoom=THREE.MathUtils.lerp(camera.zoom,desired.zoom*viewportBoost,speed);camera.updateProjectionMatrix();camera.projectionMatrix.elements[13]+=verticalShift;camera.projectionMatrixInverse.copy(camera.projectionMatrix).invert();camera.lookAt(target);for(const[name,mesh]of Object.entries(glows)){mesh!.material.opacity=THREE.MathUtils.lerp(mesh!.material.opacity,(desiredRoom==='all'||desiredRoom===name)?(preview ? .62 : .5):.07,reduced?1:.18);mesh!.material.color.set(preview?'#b9e7cf':'#d7e7d8');}if(glows.bedroom)glows.bedroom.material.color.set(deviceState.acOn?'#d3e8ea':'#d7e7d8');const liveLight=deviceState.lightOn?.35+Math.max(0,Math.min(100,deviceState.brightness))/100*6:.08;livingLight.intensity=THREE.MathUtils.lerp(livingLight.intensity,liveLight,reduced?1:.2);bedLight.color.set(deviceState.acOn?'#bcdcff':'#ffe2b8');bedLight.intensity=THREE.MathUtils.lerp(bedLight.intensity,deviceState.acOn?Math.max(2.4,(30-deviceState.temperature)*.45):.25,reduced?1:.18);const closed=1-Math.max(0,Math.min(100,deviceState.curtainOpen))/100;const curtainScale=.25+closed*1.65;curtainLeft.scale.x=THREE.MathUtils.lerp(curtainLeft.scale.x,curtainScale,reduced?1:.2);curtainRight.scale.x=THREE.MathUtils.lerp(curtainRight.scale.x,curtainScale,reduced?1:.2);curtainLeft.position.x=THREE.MathUtils.lerp(curtainLeft.position.x,-1.38+closed*.55,reduced?1:.2);curtainRight.position.x=THREE.MathUtils.lerp(curtainRight.position.x,1.38-closed*.55,reduced?1:.2);(door.material as THREE.MeshStandardMaterial).color.lerp(deviceState.locked?lockedColor:unlockedColor,reduced?1:.2);renderer.render(scene,camera);frame=requestAnimationFrame(draw);};draw();
